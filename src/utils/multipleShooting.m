function [v, iter, diff, corrected_flag, initial_constraint_violation, initial_step_size, T0] = multipleShooting(x_init, T0, mu)
    ...
    while iter <= max_iter
        F = zeros(6*nseg+2,1);
        DF = zeros(6*nseg+2,6*nseg);
        % F = zeros(6*nseg+3,1);
        % DF = zeros(6*nseg+3,6*nseg);
