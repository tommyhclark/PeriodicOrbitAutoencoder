...
l2 = const.l2;
config = readyaml("HaloFamilyPlanarSupplement/halo_config.yaml");
...
parfor ii=1:length(decoded_states)
    fprintf("%i \n", ii)
    [v,iter,diff, corrected_flag, initial_constraint_violation, initial_step_size] = multipleShootingVariablePeriod(squeeze(decoded_states(ii,:,1:6)),decoded_periods(ii),mu);
    if corrected_flag
...
%% Performance Plots - Correctability
...
diff_period = abs(test_periods-decoded_periods);
figure;
min_val = min(diff_period(diff_period > 0));
max_val = max(diff_period);
bin_edges = logspace(log10(min_val), log10(max_val), 31);
bin_centers = sqrt(bin_edges(1:end-1) .* bin_edges(2:end)); % geometric centers for log bins
[N, ~] = histcounts(diff_period, bin_edges, 'Normalization', 'probability');
cmap = parula(numel(bin_centers));
norm_vals = (log10(bin_centers) - log10(min_val)) / (log10(max_val) - log10(min_val));
hold on
for i = 1:numel(bin_centers)
...
%% Differences in inpute states from decoded_states
state_differences = test_states-decode_states;
figure;
