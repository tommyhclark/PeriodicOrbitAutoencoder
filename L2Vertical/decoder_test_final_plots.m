...
xShift = const.l2;
l2 = const.l2;
config = readyaml("L2Vertical/config.yaml");
...
axis(ax1,'equal');
xlim(ax1,[-1.2 1.2]); ylim(ax1,[-1.5 1.5]); zlim(ax1,[-1.5 1.5]);
view(ax1, [90 0]);
daspect(ax1,[1 1 1]);
...
ax8 = subplot(2,3,5);
scatter(ax8, latent_data, zdot_apolune, 20, cmap(1:length(latent_data),:), 'filled');
xlabel(ax8,'Latent Variable','FontSize',20,'Interpreter','latex');
ylabel(ax8,'$L2$ $\dot{Z}$ Value','FontSize',20,'Interpreter','latex');
...
%%
%% Plot 1 - Aligned 3-on-2 (Spatial & Latent)
% clc
z_apolune = decoded_states(:,1,3);
cmap = jet(Norb);
fig = figure('Color','w','Units','pixels');
...
% Create a 2x6 grid: 
% Top Row uses spans of 2 columns (2+2+2=6)
% Bottom Row uses spans of 3 columns (3+3=6)
t = tiledlayout(2, 6, 'Padding', 'compact', 'TileSpacing', 'loose');

% Pre-calculate Moon sphere data
moon_radius = const.moon_R;
[Xm,Ym,Zm] = sphere(50);
Xm = moon_radius*Xm + (1-mu); 
Ym = moon_radius*Ym; 
...
% =========================================================================
% TOP ROW: 3 Views (Spatial)
% =========================================================================

% --- View 1: 3D Side View (XZ plane) ---
ax1 = nexttile(1, [1 2]); 
hold(ax1,'on');
for ii = 1:200:Norb
    states_out = squeeze(decoded_states(ii,:,:));
...
zlabel(ax1,'Z','Interpreter','latex','FontSize',20);
set(ax1, 'LineWidth', 2, 'FontSize', 20, 'FontWeight', 'bold', 'TickLabelInterpreter', 'latex');
axis(ax1,'equal'); xlim(ax1,[-1.2 1.2]); ylim(ax1,[-1.5 1.5]); zlim(ax1,[-1.5 1.5]);
view(ax1, [90 0]); daspect(ax1,[1 1 1]);

% --- View 2: XY Plane (Top View) ---
ax2 = nexttile(3, [1 2]);
allKids = get(ax1,'Children');
copyobj(allKids, ax2);
view(ax2, [0 0]);
...
zlabel(ax2,'Z','Interpreter','latex','FontSize',20);
set(ax2, 'LineWidth', 2, 'FontSize', 20, 'FontWeight', 'bold', 'TickLabelInterpreter', 'latex');
axis(ax2,'equal'); xlim(ax2, xlim(ax1)); ylim(ax2, ylim(ax1)); zlim(ax2, zlim(ax1));
daspect(ax2,[1 1 1]);

% --- View 3: XZ Plane (Side View) ---
ax3 = nexttile(5, [1 2]);
copyobj(allKids, ax3);
view(ax3, [0 90]);
xlabel(ax3,'X','Interpreter','latex','FontSize',20);
...
% =========================================================================
% BOTTOM ROW: 2 Latent Variable Plots
% =========================================================================

% --- Latent vs Period ---
ax4 = nexttile(7, [1 3]); % Starts at tile 7, spans 3 columns
scatter(ax4, latent_data, decoded_periods, 20, cmap(1:length(latent_data),:), 'filled');
xlabel(ax4,'Latent Variable','FontSize',20,'Interpreter','latex');
ylabel(ax4,'Orbit Period','FontSize',20,'Interpreter','latex');
set(ax4, 'LineWidth', 2, 'FontSize', 20, 'FontWeight', 'bold', 'TickLabelInterpreter', 'latex');
box(ax4, 'on'); grid(ax4, 'on');

% --- Latent vs Jacobi Constant ---
ax5 = nexttile(10, [1 3]); % Starts at tile 10, spans 3 columns
scatter(ax5, latent_data, jacobiConstant(decoded_states(:,1,1:6),mu), 20, cmap(1:length(latent_data),:), 'filled');
xlabel(ax5,'Latent Variable','FontSize',20,'Interpreter','latex');
ylabel(ax5,'Jacobi Constant','FontSize',20,'Interpreter','latex');
